...
<body>
	...
	<div id="main">
		...
		<table class="inputs">
			<tr><td>学籍番号</td><td>：<input type="text" value="${st_data.st_no}" disabled></td></tr>
			<tr><td>氏名（必須）</td><td>：<input type="text"  value="${st_data.st_name}" disabled/></td></tr>
			<tr><td>フリガナ（必須）</td><td>：<input type="text" name="st_kana" id="cst_kana" value="${st_data.st_kana}" class="required" maxlength="40" ></td></tr>
			<tr><td>学科</td><td>：
		<select name="class">
			<c:forEach var="item" items="${classlist}">
			<c:if test="${st_data.class_no == item.class_no}">
			...
			<c:if test="${st_data.year == status.index}">
			...
			<tr><td colspan="2"><input type="checkbox" onclick="this.form.pass.value=this.checked ? 'true' : 'false'">パスワードを初期化する</td></tr>
			<tr><td colspan="2"><input type="submit" value="送信" /></td></tr>
		</table>
		<input type="hidden" name="st_no" value="${st_data.st_no}">
