...
<body>
	...
	<div id="main">
		...
		${class}
		<form method="get" action="mg_testsearchst">
		<p>実施団体：
		<select name="associ">
		...
		<input type="button" value="リセット" onClick="location.href='mg_testsearchst'" />
		</p>
		</form>
		
		<form name="limitno" method="get" action="mg_testsearchst_get">
		...
		<table border="3" class="hitable">
			...
			<c:forEach var="item" items="${list}" varStatus="status">
				<tr><td><a href="mg_testsearchst_get?test_no=${item.test_no}">${item.test_no}</a></td><td>${item.test_name}</td><td>${item.associ_name}</td></tr>
			</c:forEach>
			</table>
		</form>
		</c:if>
