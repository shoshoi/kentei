...
<body>
	...
	<div id="main">
		...
		${class}
		<form method="get" action="mg_maintestmaster">
		<p>学科：
		<select name="class">
		...
		<input type="button" value="リセット" onClick="location.href='mg_maintestmaster'" />
		</p>
		</form>
		
		<form name="limitno" method="get" action="mg_maintestmaster_get">
		<p>学籍番号：<input type="text" size="10" name="st_no">
		　<input type="submit" value="送信" /></p>
		</form>
		
		<c:if test="${!empty list}">
		<form method="post" action="mg_maintestmaster_delete">
		<table border="3" class="hitable">
			<tr><th>学籍番号</th><th>氏名</th><th>学科</th><th>学年</th></tr>
			<c:forEach var="item" items="${list}" varStatus="status">
				<tr><td><a href="mg_maintestmaster_get?st_no=${item.st_no}" >${item.st_no}</a></td><td>${item.st_name}</td><td>${item.class_name}</td><td>${item.year}</td></tr>
			</c:forEach>
			</table>
		</form>
		</c:if>
