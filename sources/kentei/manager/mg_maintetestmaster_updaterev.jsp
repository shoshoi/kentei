...
<body>
	...
	<div id="main">
		...
		<table class="inputs">
		<c:if test="${param.insertflg == 'false'}">
			<tr><td>検定番号</td><td>：<input type="text" value="${param.test_no}" disabled></td></tr>
			<input type="hidden" name="test_no" value="${param.test_no}">
			<input type="hidden" name="insertflg" value="false">
		</c:if>
		<c:if test="${param.insertflg == 'true'}">
			<tr><td>検定番号（必須，8桁）</td><td>：<input type="text" name="test_no" id="ctest_no" value="${param.test_no}" class="required number" minlength="8" maxlength="8" /></td></tr>
			<input type="hidden" name="insertflg" value="true">
		</c:if>
		<tr><td>検定名（必須）</td><td>：<input type="text" name="test_name" id="ctest_name" value="${param.test_name}" class="required" minlength="1" maxlength="50" /></td></tr>
		<tr><td>実施団体</td>
			<td>：
